...
class MapColoringConstraint(Constraint[str, str]):
    def __init__(self, place1: str, place2: str) -> None:
        # Constraint.__init__([place1, place2]) 클래스 자체 이름으로 사용할 수 있음
        super().__init__([place1, place2])
        self.place1: str = place1
        self.place2: str = place2

    def satisfied(self, assignment: Dict[str, str]) -> bool:
        # 두 지역 중 하나가 색상이 할당되지 않았다면 색상 충돌은 발생하지 않음
        if self.place1 not in assignment or self.place2 not in assignment:
            return True
        # place1과 place2에 할당된 색상이 다른지 확인
        return assignment[self.place1] != assignment[self.place2]

if __name__ == "__main__":
    variables: List[str] = ["웨스턴 오스트레일리아", "노던 준주", "사우스 오스트레일리아", "퀸즐랜드", "뉴사우스웨일스", "빅토리아", "테즈매니아"]
    ...
    csp.add_constraint(MapColoringConstraint("빅토리아", "테즈매니아"))
