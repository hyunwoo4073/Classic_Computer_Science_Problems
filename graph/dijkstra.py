...
def dijkstra(wg: WeightedGraph[V], root: V) -> Tuple[List[Optional[float]], Dict[int, WeightedEdge]]:
    ...
    # 처음에는 거리(distances)를 알 수 없음
    distances: List[Optional[float]] = [None] * wg.vertex_count
    distances[first] = 0 # 루트(root)에서 루트 자신의 거리는 0임
    ...
    while not pq.empty:
        ...
        dist_u: float = distances[u] # 이 정점에 대한 거리를 이미 알고 있음
        # 이 정점에서 모든 에지 및 정점을 살펴봄
...
# 정점 start에서 end까지 가는 에지 리스트를 반환
