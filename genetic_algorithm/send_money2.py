...
class SendMoreMoney2(Chromosome):
    ...
    def mutate(self) -> None: # 두 문자의 위치를 스왑
        idx1, idx2 = sample(range(len(self.letters)), k=2)
...
if __name__ == "__main__":
    ...
    initial_population: List[SendMoreMoney2] = [SendMoreMoney2.random_instance() for _ in range(1000)]
    ga: GeneticAlgorithm[SendMoreMoney2] = GeneticAlgorithm(initial_population=initial_population, threshold=1.0, max_generations=1000, mutation_chance=0.2, crossover_chance=0.7, selection_type=GeneticAlgorithm.SelectionType.SOFT_TOURNAMENT)
    result: SendMoreMoney2 = ga.run()
    print(result)
